...
var num_pag = 1;
var last_search = '';
var categoria = require('../combos/movies-categoria');
...
jQuery.fn.vtexSmartResearch=function(opts)
{
	...
	var fn=
	{
		getUrl:function(scroll)
		{
			...
			if(s){
                if( isPagination )
                    return currentSearchUrl.replace(/PageNumber=[0-9]*/,"PageNumber="+num_pag);
                else
                    return currentSearchUrl.replace(/PageNumber=[0-9]*/,"PageNumber="+currentPage);
            }else{
                if( isPagination )
                    return (searchUrl+urlFilters).replace(/PageNumber=[0-9]*/,"PageNumber="+num_pag);
                else
		...
		{
			var typeMovil = fn.getTypeMovil();
			var customapp = fn.getCookie("customapp") || location.search.indexOf('customapp');
            if( typeMovil == 'iOS' && $('body').is('.ff-pagination') && customapp !== "" && customapp != -1 ){
                ...
                $(document).on("click", '#paginador .pag-hidden span', function(m) {
                    num_pag = $(this).html();
                    $('.cat-container__products__vitrine #counter-products .pag-actual').html(num_pag);
                    ...
                    pageJqxhr=jQuery.ajax({
                        ...
                        success:function(data)
                        {
                            if(data.trim().length<1)
                            {
								...
								data = data.replace('<META NAME="ROBOTS" CONTENT="NOINDEX, FOLLOW">','');
                                currentItems.parent().empty().html(data);
                            }
                            currentStatus=true;
                            elemLoading.remove();
                            ...
                            console.log("después");
                            fn.animatePagination(false);
                            setTimeout(function(){$(document).trigger('ajaxStop')},500);
                        }
                    });
                    currentPage++;
                });
        ...
        animatePagination:function(animar){
            if(animar){
	...
	var fns=
	{
		...
		filterAjaxSuccess:function(data)
		{
			...
			var typeMovil =  fn.getTypeMovil();
			var customapp = fn.getCookie("customapp") || location.search.indexOf('customapp');
            ( typeMovil === "iOS" && customapp !== "" ? ( ($('.sr_selected').length) ? categoria.funcionalidadesCategoria.setPagination(false,last_search): categoria.funcionalidadesCategoria.setPagination(true,last_search)) : "" );
		...
		updateContent:function($data)
		{
			...
			(shelfPage.length>0?shelfPage:options.emptySearchElem).slideUp(600,function(){
				...
				{
					shelf.hide();
					loadContentE.empty().append(shelf);
